...
Use this pack when the user provides a specific Oracle solution link, asks to recreate a known Oracle diagram, or wants the closest possible match to an Oracle reference architecture.
...
- HA or DR posture
- constraints and best practices that the diagram should make visible
...
5. Choose one primary bundled reference and only the smallest useful set of supporting references.
6. Draft the initial diagram.
...
- Constraints and best practices:
  ...
  Prefer private integration paths for multicloud or on-premises connectivity and show governance or observability when the diagram claims enterprise readiness.
- Recommended follow-up questions:
  ...
  Ask whether the diagram should emphasize RAG, conversational interfaces, or model-training infrastructure.
- Recreation prompt:
  Create a physical OCI architecture for a secure self-service generative AI platform with separate sandbox and production environments inside one OCI region. Show Data Science and GPU playground resources for experimentation, OKE and OCI Generative AI for production inference, Oracle AI Database plus Object Storage as the knowledge layer, and OCI Events, Streaming, Connector Hub, Functions, API Gateway, and DevOps for orchestration and promotion. Preserve hybrid or multicloud touchpoints only when requested and keep enterprise governance and observability visible.
- Best local starting points:
  Primary imported reference: `ai-llm-workflow-architecture.drawio`
  Supporting imported reference: `oke-architecture-diagram.drawio`
  Strong seed spec: `assets/examples/specs/oke-genai-rag.json`
...
- Recreation prompt:
  Create a physical OCI architecture for a cloud-native DICOM store with an on-premises medical imaging facility feeding OCI through Roving Edge Device, FastConnect, and API Gateway. In OCI, show a DICOM store composed of Object Storage, Orthanc, and OCI Database with PostgreSQL, plus separate OKE-backed inference and training environments that read from the store. Keep the design hybrid, secure, and AI-ready without adding unrelated services.
- Best local starting points:
  Primary imported reference: `oke-architecture-diagram.drawio`
  Supporting imported reference: `hub-spoke-oci.drawio`
...
- Recreation prompt:
  Create a physical OCI architecture for Oracle WebLogic Server on OKE using a Marketplace-style deployment. Show regional public and private subnets, a bastion tier, an administrative host tier, a private OKE tier with WebLogic and Jenkins workloads, a public load balancer for the WebLogic cluster, a private load balancer for Jenkins and admin access, OCI Registry, and a private File Storage tier shared across pods. Keep the separation between runtime, administration, and storage explicit.
- Best local starting points:
  Primary imported reference: `oke-architecture-diagram.drawio`
  Supporting imported references: `architecture-use-bastion-service.drawio`, `mushop-infrastructure.drawio`
...
- Recommended follow-up questions:
  ...
  Ask whether the diagram should emphasize one public and one private subnet or a richer enterprise landing zone around them.
- Recreation prompt:
  Create a physical OCI architecture for GitOps with Argo CD on OKE. Show a simple OCI region with a VCN, one public ingress subnet with a load balancer, one private subnet with an OKE cluster, Argo CD in its own namespace, application namespaces, NAT and Service Gateway egress, and a container registry path. Keep Git as the external source of truth and keep the control plane private behind the load balancer.
- Best local starting points:
  Primary imported reference: `oke-architecture-diagram.drawio`
...
- Recommended follow-up questions:
  ...
  Ask whether the diagram should highlight Active Data Guard versus basic Data Guard.
...
- Recreation prompt:
  Create a physical OCI architecture for a modern application on OKE with separate private service tiers for OCI Database with PostgreSQL, OCI Cache with Redis or Valkey, and OCI Search with OpenSearch. Show internet ingress through WAF, optional hybrid access through DRG and FastConnect, Bastion for administration, and OCI service access through a Service Gateway. If HA is requested, show the managed data services with replica or leader roles spread across fault domains while keeping the application platform clear and uncluttered.
- Best local starting points:
  Primary imported reference: `oke-architecture-diagram.drawio`
  Supporting imported reference: `hub-spoke-oci.drawio`
  Strong seed spec: `assets/examples/specs/oke-multidatabase-modern-app.json`
